...
VOCAB = [  # keyword → the ORPHAcode used on the site (REG_CODE_NAMES in build-demo.py); None = the dysmelia family in general
    ...
    # ORPHA:1027 was withdrawn by Orphanet (moved to tetraamelia-multiple malformations syndrome);
    # non-syndromic amelia is ORPHA:294925, which is what the card carries.
    (r"\bamelia\b", "294925"), (r"\bphocomelia\b", "2879"), (r"\bmeromelia\b", None), (r"\bhemimelia\b", "2130"),
    (r"\bectrodactyly|split[- ]hand|split[- ]foot|cleft hand|\bSHFM\b", "2440"), (r"\bsymbrachydactyly", "1570"), (r"\bbrachydactyly", None),
    (r"crossed polysyndactyly", "2935"), (r"\bpolydactyly", "2913"), (r"\bsyndactyly", "90025"),
    ...
    (r"proximal femoral focal|\bPFFD\b", "633228"),
    (r"femoral (agenesis|deficien|hypoplasia)|congenital short femur", "1987"),
    (r"humeral (agenesis|deficien|hypoplasia)", "294973"),
    ...
    (r"(hypoplastic|absent|floating) thumbs?|thumbs? (hypoplasia|aplasia|deficien)|pollici[sz]ation", "294988"),
    (r"triphalangeal thumbs?|thumb reconstruction", None),
    (r"macrodactyly|megadactyly|oligodactyly|ectromelia|\bdimelia\b|ulnar dimelia", None),
    # The forms inside our three group codes, each with the term the literature uses for it. A paper
    # on one form carries both its own code and its group's, which is the broad reading the
    # conditions page states: these rules add to the group rules above rather than replacing them.
    (r"acheiropod(ia|y)", "931"),
    (r"\bacheiria\b|absence of (the )?hand and wrist", "294983"),
    (r"\bapodia\b|absence of (the )?foot and ankle", "294986"),
    (r"absence of (both |the )*forearm and hand|radio-?ulnar terminal transverse", "294979"),
    (r"absence of (both |the )*lower leg and foot|tibiofibular terminal transverse", "294981"),
    (r"complete hemimelia", "498491"),
    (r"intercalary", "294927"),
    (r"longitudinal (limb )?(deficien|defect|meromelia)", "498457"),
    (r"hyperphalangy|supernumerary phalan", "295002"),
    (r"synpolydactyly|syndactyly type (2|II)\b", "93403"),
    (r"zygodactyly|syndactyly type (1|I)\b|\bSD1\b", "93402"),
    (r"syndactyly type (3|III)\b|\bSD3\b", "93404"),
    (r"syndactyly type (4|IV)\b|polysyndactyly, haas|haas type", "93405"),
    (r"syndactyly type (5|V)\b|\bSD5\b", "93406"),
    (r"syndactyly type (6|VI)\b|mitten hand", "295012"),
    (r"syndactyly type (8|VIII)\b", "2498"),
    (r"syndactyly type (9|IX)\b|mesoaxial synostotic|malik-percin", "157801"),
    (r"postaxial polydactyly type A\b", "93334"),
    (r"postaxial polydactyly type B\b", "93335"),
    (r"postaxial polydactyly", "498467"),
    (r"preaxial polydactyly type (2|II)\b|\bPPD2\b", "93336"),
    (r"preaxial polydactyly type (3|III)\b|\bPPD3\b", "93337"),
    (r"preaxial polydactyly type (4|IV)\b|\bPPD4\b", "93338"),
    (r"preaxial polydactyly type (1|I)\b|\bPPD1\b", "93339"),
    (r"preaxial polydactyly", "498464"),
    (r"central polydactyly|mesoaxial polydactyly", "295004"),
